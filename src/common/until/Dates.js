...
export function dateFormat(date, format = 'YYYY-MM-DD') {
  ...
  return format.replace('YYYY', date.getFullYear()).replace('MM', month).replace('DD', day).replace('HH', hour).replace('NN', minute).replace('SS', second);
}
...
export function secondsToCnDesc(seconds) {
  if (seconds === 0 || seconds === '0') return '0' + $_L.get('秒');
  if (!seconds) return '';
  let day = parseInt(seconds / (60 * 60 * 24));
  let hour = parseInt((seconds % (60 * 60 * 24)) / (60 * 60));
  let minute = parseInt(((seconds % (60 * 60 * 24)) % (60 * 60)) / 60);
  let second = parseInt(((seconds % (60 * 60 * 24)) % (60 * 60)) % 60);
  let desc = [];
  day > 0 && desc.push(day, $_L.get('天'));
  hour > 0 && desc.push(hour, $_L.get('时'));
  minute > 0 && desc.push(minute, $_L.get('分'));
  second > 0 && desc.push(second, $_L.get('秒'));
  return desc.join('');
}

/**
 * 获取当前年月日
 * @params D 一个时间对象,如new Date(),不传获取当前年月日
 * @returns {String} 2020-05-20
 */
